...
def ridgeplot(
    ...
    both ``samples`` and ``densities`` arguments, a :py:exc:`ValueError`
    exception will be raised! One of these arguments should always remain set
    to None. See ``samples`` and ``densities`` bellow for more information.

    Args:
        samples
            If ``samples`` data is specified, Kernel Density Estimation (KDE)
            will be computed. See ``kernel``, ``bandwidth``, and ``kde_points``
            for more details and KDE configuration options.
        densities
            If ``densities`` arrays are specified instead, the KDE step will be
            skipped and all associated arguments ignored. Each density array
            should have shape :math:`(2, N)`, but :math:`N` may vary with each
            array.
        kernel
            The Kernel to be used during Kernel Density Estimation. The default
            is a Gaussian Kernel (``"gau"``). Choices are:\n
            - ``"biw"`` for biweight
            - ``"cos"`` for cosine
            ...
            - ``"uni"`` for uniform
        bandwidth
            The bandwidth to use during Kernel Density Estimation. The default
            is ``normal_reference``. Choices are:\n
            ...
            - If a callable is given, it's return value is used. The callable
              ...
              return a float, where:\n
              - ``x``: the clipped input data
              - ``kern``: the kernel instance used
        kde_points
            This argument controls the points at which KDE is computed. If an
            int value is passed (default), the densities will be evaluated at
            ``kde_points`` evenly spaced points between the min and max of each
            set of samples. However, you may also specify a custom range by
            instead passing an array of points. This array should be
            one-dimensional.
        colorscale
        ...
        colormode
            ...
            calculating the ``colorscale`` midpoint of each trace. The default
            is mode is ``"mean-means"``. Choices are:\n
            ...
            - ``"mean-means"`` - uses the min-max normalized (weighted) mean of
              ...
              densities, respectively.
        coloralpha
            If None (default), this argument will be ignored and the
            transparency values of the specifies color-scale will remain
            untouched. Otherwise, if a float value is passed, it will be used
            to overwrite the transparency (alpha) of the color-scale's colors.
        labels
            A list of string labels for each trace. The default value is
            None, which will result in auto-generated labels of form "Trace n".
            If, instead, a list of labels is specified, it must be of the same
            size/length as the number of traces.
        linewidth
            The traces' line width (in px).
        spacing
            The vertical spacing between density traces, which is defined in
            units of the highest distribution (i.e. the maximum y-value).
        show_annotations
    ...
    Returns:
        :py:class:`plotly.graph_objects.Figure`
            A Plotly :py:class:`~plotly.graph_objects.Figure` with a ridgeline
            plot. You can further customize this figure to your liking (e.g.
            using the :py:meth:`~plotly.graph_objects.Figure.update_layout()`
            method).
