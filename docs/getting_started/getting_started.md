...
![ridgeline plot example using the ridgeplot Python library](../_static/img/example_basic.png)

## Fully configurable

In this example, we will try to replicate the first ridgeline plot in this [_from Data to Viz_ post](https://www.data-to-viz.com/graph/ridgeline.html). The example in the post was created using the _"Perception of Probability Words"_ dataset (see {py:func}`~ridgeplot.datasets.load_probly()`) and the popular [ggridges](https://wilkelab.org/ggridges/) R package. In the end, we will see how the `ridgeplot` Python library can be used to create a (nearly) identical plot.
...
:::{tab-item} Target/reference
The target reference from the [_from Data to Viz_ post](https://www.data-to-viz.com/graph/ridgeline.html):
![reference ridgeline plot of the probly dataset from data to viz](../_static/img/example_probly_original.png)
:::
...
This example demonstrates how you can also draw multiple traces per row in your ridgeline plot.

:::{dropdown} {octicon}`image;1.2em` Preview of the final result
Here is a static image of the final result to help you visualize the ridgeline plot that we are working towards:
![ridgeline plot of the Lincoln Weather dataset using the ridgeplot Python library](../_static/img/example_lincoln_weather.png)
:::
...
samples = [
    [
        df[df.index.month_name() == m]["Min Temperature [F]"],
        df[df.index.month_name() == m]["Max Temperature [F]"],
    ]
    for m in months
]
```
